"""
phase2_train_b2.py — Fase 2 B2: RandAugment moderado.

Mesma config do baseline (lr=0.0005, dropout=0.1, onecycle, AdamW)
com augmentation="randaug_nocolor" e imagens segmentadas.
"""
...
CHECKPOINT_PATH = os.path.join(os.path.dirname(__file__), "best_phase2_B2.pth")
...
def train():
    ...
    print(f"Experiment: {EXPERIMENT_ID} — RandAugment NoColor")
    ...
    print("\n--- COMPARATIVO ---")
    print(f"{'Métrica':<20} {'Baseline B0':>12} {'B1 RandAug':>12} {'B2 NoColor':>12} {'Delta B2-B0':>12}")
    print("-" * 70)
    print(f"{'Val Accuracy':<20} {'98.17%':>12} {'97.78%':>12} {val_metrics['accuracy']*100:>11.2f}% {(val_metrics['accuracy']-0.9817)*100:>+11.2f}pp")
    print(f"{'Holdout Accuracy':<20} {'70.69%':>12} {'68.97%':>12} {holdout_metrics['accuracy']*100:>11.2f}% {(holdout_metrics['accuracy']-0.7069)*100:>+11.2f}pp")
    print(f"{'Holdout F1':<20} {'0.6994':>12} {'0.6627':>12} {holdout_metrics['f1']:>12.4f} {holdout_metrics['f1']-0.6994:>+12.4f}")

    baseline_per_class = {"Ferrugem": 0.70, "Mancha-alvo": 0.89, "Mosaico": 0.57, "Olho-de-rã": 0.62, "Oídio": 0.75}
    b1_per_class = {"Ferrugem": 0.50, "Mancha-alvo": 0.89, "Mosaico": 0.57, "Olho-de-rã": 0.62, "Oídio": 0.81}

    print(f"\n{'Classe':<15} {'B0':>8} {'B1':>8} {'B2':>8} {'B2-B0':>8}")
    print("-" * 50)
    for cls in sorted(per_class.keys()):
        ...
        if pc["total"] > 0:
            ...
            b1 = b1_per_class.get(cls, 0)
            print(f"{cls:<15} {base*100:>7.0f}% {b1*100:>7.0f}% {acc*100:>7.0f}% {(acc-base)*100:>+7.0f}pp")
